...
/*
 * 函 数:mq_sm_get_data
 * 功 能:从mq_queue中取出一条消息保存到msg_item中
 */
bool mq_sm_get_data(msg_item_t* msg_item, mq_queue_t* mq_queue)
{
    ...
    while (1)
    {
        ...
        if (ret == MSG_CHECK_RET_OK)
        {
            ...
            cur_timestemp = get_cur_timestamp();            //当前时间戳
 ...
 * 功 能:创建队列
 * 参 数:mq_queue, 
 * 注 意:mq_queue是一个队列的数据结构，根据mq_queue中的qname属性创建好队列之后，把生成的一些属性再保存到mq_queue中
 */
bool mq_sm_creat_db(mq_queue_t *mq_queue)
{
    char    queue_db_path[MAX_DATA_PATH_NAME_LEN + 1];  //队列文件夹的路径
    char    rtag_fname[MAX_FULL_FILE_NAME_LEN + 1];     //rtag文件的路径
    char    wtag_fname[MAX_FULL_FILE_NAME_LEN + 1];     //wtag文件的路径
    char    write_db_fname[MAX_FULL_FILE_NAME_LEN + 1]; //db文件的路径
    ...
    GEN_QUEUE_PATH_BY_QNAME(queue_db_path, mq_queue->qname);    //路径加上队列名合并成一个队列路径(这是个文件夹)
