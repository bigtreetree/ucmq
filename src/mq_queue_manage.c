 ...
 *功 能:
 *注 意:会先检查文件是否存在，如果不存在,则直接在遍历目录时返回（因为空目录中只有.和..,没有其它）
 */
bool mq_qm_open_store(void)
{
    ...
    /* Open data path */
	//打开目录
    if ((p_dir = opendir(g_mq_conf.data_file_path)) == NULL)    //./mq_data/data
    {
...
/*
 * 函 数:mq_qm_push_item
 * 功 能:向队列mq_queue中推入一条消息msg_item
 * 返回值:队列如果满了则返回QUEUE_FULL
 *       
 */
int mq_qm_push_item(mq_queue_t *mq_queue, msg_item_t *msg_item)
{
    ...
    msg_item->magic_num1 = MSG_HEAD_OF_FIRST_MAGIC_NUM; //magic code 有什么用
    msg_item->magic_num2 = MSG_HEAD_OF_SECOND_MAGIC_NUM;

    /* then def_max_queue is equal to 0 no limit */
    //mqxque不等于０时，说明队列有个最大长度限制
    if (mq_queue->maxque != 0)
    {
        //存入的消息个数　－　取出的消息个数　＋　１是否大于限制值
...
/*
 * 函 数:mq_qm_pop_item
 * 功 能:从队列mq_queue中弹出一条消息，保存到msg_item中 
 */
