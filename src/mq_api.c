...
static int qname_check(const char* qname)
{
    ...
    const int qname_len = (qname != NULL) ? strlen(qname) : -1; //队列名称长度
...
bool mq_store_put(const char *qname, const char *msg, int msg_len)
{
    ...
    //检查名字是否符合要求(名称不能过长，而且只能由数字和大小写字母构成)
...
/*
 * 函 数:mq_store_get
 * 功 能:从队列qname 中获取消息，把获取到的消息存到长度为msg_len的msg指向的buf中
 */
